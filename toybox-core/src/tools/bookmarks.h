...
//
// Sixteen per book, eight bytes each: this is a shelf for the handful of
// places worth going back to, not a highlighting system.
#pragma once
#include <stdint.h>
#include <string.h>
...
// "TBM1", u16 count, then MAX entries. Fixed length, so a save is one write
// and a load is one read, and a truncated file is refused rather than guessed
// at -- there is nothing here worth recovering half of.
inline constexpr int FILE_BYTES = 6 + MAX * 8;
...
inline int load(ToolsHost& h, const char* file, Mark* out) {
  ...
  if (memcmp(buf, "TBM1", 4) != 0) return 0;
  int n = buf[4] | (buf[5] << 8);
  if (n > MAX) n = MAX;
  for (int i = 0; i < n; i++) {
    const uint8_t* e = buf + 6 + i * 8;
