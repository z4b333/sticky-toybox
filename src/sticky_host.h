...
class StickyCanvas : public ToolsCanvas {
 ...
 private:
  ...
  static int scaleOf(TSize sz) {
    switch (sz) {
      // 235 DPI, so these are millimetres as much as pixels: 1.9, 2.6, 3.4,
      ...
      // and the smallest moved again -- 16 to 18 -- when holding it closer
      // turned out to be how people were reading the sub-labels.
      case TS_SMALL: return 18;
...
class StickyHost : public ToolsHost {
 public:
  ...
  int sdRecipes(char names[][RECIPE_NAME_LEN], int max) override;
  int sdTextFiles(const char* dir, char names[][RECIPE_NAME_LEN], int max) override;
  bool coverFromBmp(const char* file) override;
